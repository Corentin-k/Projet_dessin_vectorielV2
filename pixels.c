 ...
 */
void pixel_polygon(Polygon * polygon, Pixel*** pixel, int* nb_pixels,char* color) {
    ...
    for (int i = 0; i < polygon->n - 1; i++) {
        ...
        Pixel** line_pixels;
        int nb_line_pixels;
        Shape* l=create_line_shape(x1,y1,x2,y2,color);
...
Pixel** create_shape_to_pixel(Shape * shape, int* nb_pixels) {
    ...
    switch (shape->shape_type) {
        ...
        case CIRCLE:
            ...
            break;
        case POLYGON:
            pixel_polygon(shape->ptrShape, &pixel, nb_pixels,shape->color);
            break;
    }
    return pixel;
}
